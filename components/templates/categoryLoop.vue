<template>
	<v-container class="col-12 loop_container">
		...
		<v-row
			...
			:class="[
				post.status === '1'
					? 'blue-grey darken-3 lighten-4 mb-2 align-center font-podkova-bold'
					: 'brown darken-2 mb-2 align-center font-podkova-bold'
			]"
		>
			<v-col class="col-3 white--text">
				{{ post.title }}
			</v-col>
			<v-col class="col-4 white--text" v-html="post.short_desc"> </v-col>
			...
			<v-col class="col-2 text-right">
				<v-btn class="deep-orange darken-2" :to="`/admin/${post.post_type}/${post.id}`"
					>Edit
					<v-icon dark right>mdi-tooltip-edit</v-icon>
				</v-btn>
			</v-col>
		</v-row>
	</v-container>
